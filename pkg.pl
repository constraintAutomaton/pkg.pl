...
report_installation_step(do_nothing(dependency(Name, DependencyTerm), L), _, do_nothing(dependency(Name, DependencyTerm), L)-success).


report_installation_step(install_locked_dependency(D, L), ResultMessages, install_locked_dependency(D, L)-Message):-
    report_installation_step(install_dependency(D), ResultMessages, _-Message).

report_installation_step(install_dependency(dependency(Name, DependencyTerm), L), ResultMessages, install_dependency(dependency(Name, DependencyTerm, L))-Message):-
    memberchk(result(Name, Message, _), ResultMessages).
